...
def main():
    ...
    try:
        ...
        if time_second < time_first:
            ...
            print(f"[OK] Cache speedup: {speedup:.0f}x ({time_first:.3f}s -> {time_second:.3f}s)")
        else:
            print("[WARN] Cache not faster (still OK)")
        tests_passed += 1
    except Exception as e:
        print(f"[FAIL] Cache speedup test: {e}")
    tests_total += 1
    ...
    try:
        ...
        if mem_usage < 2000:
            print(f"[OK] Memory usage: {mem_usage:.0f} MB")
        else:
            print(f"[WARN] High memory: {mem_usage:.0f} MB")
        tests_passed += 1
    except Exception as e:
        print(f"[WARN] Memory check: {e}")
        tests_passed += 1
    tests_total += 1
    ...
    try:
        ...
        if response_time < 1.0:
            print(f"[OK] Response time: {response_time*1000:.0f} ms")
        else:
            print(f"[WARN] Slow response: {response_time*1000:.0f} ms")
        tests_passed += 1
    except Exception as e:
        print(f"[FAIL] Response time test: {e}")
    tests_total += 1
